...
impl<R> Unparsable for StmtWith<R> {
    fn unparse(&self, indentation_level: usize) -> String {
        ...
        for (i, item) in self.items.iter().enumerate() {
            ...
            result.push_str(&item.unparse(indentation_level));
        }
        result.push_str(":\n");
        if let Some(type_comment) = &self.type_comment {
            result.push_str(&format!("{}# type: {}", SPACING.repeat(indentation_level + 1), type_comment));
...
impl<R> Unparsable for StmtRaise<R> {
    fn unparse(&self, indentation_level: usize) -> String {
        ...
        if let Some(exc) = &self.exc {
            result.push_str(&format!(" {}", exc.unparse(indentation_level)));
        }
...
impl<R> Unparsable for StmtTry<R> {
    fn unparse(&self, indentation_level: usize) -> String {
        ...
        if !self.finalbody.is_empty() {
            result.push_str(&format!("\n{}finally:", SPACING.repeat(indentation_level)));
...
impl<R> Unparsable for ExprBinOp<R> {
    fn unparse(&self, _: usize) -> String {
        ...
        result.push_str(&self.right.unparse(0));
        result
    }
}
...
impl<R> Unparsable for ExprDict<R> {
    fn unparse(&self, _: usize) -> String {
        ...
        for (i, (key, value)) in self.keys.iter().zip(self.values.iter()).enumerate() {
            ...
            if let Some(key) = key {
                result.push_str(&format!("{}: {}", key.unparse(0), value.unparse(0)));
            } else {
                result.push_str(&format!("{}", value.unparse(0)));
            }
        }
        result.push_str("}");
        result
...
impl<R> Unparsable for ExprListComp<R> {
    fn unparse(&self, _: usize) -> String {
        let mut result = String::new();
        result.push_str("[");
        result.push_str(&self.elt.unparse(0));
        for generator in &self.generators {
            result.push_str(&generator.unparse(0));
        }
        result.push_str("]");
        result
    }
...
impl<R> Unparsable for ExprDictComp<R> {
    fn unparse(&self, _: usize) -> String {
        let mut result = String::new();
        result.push_str("{");
        result.push_str(&self.key.unparse(0));
        result.push_str(": ");
        result.push_str(&self.value.unparse(0));
        for generator in &self.generators {
            result.push_str(&generator.unparse(0));
...
impl<R> Unparsable for ExprCall<R> {
    fn unparse(&self, _: usize) -> String {
        ...
        for (i, arg) in self.args.iter().enumerate() {
            ...
            result.push_str(&arg.unparse(0));
        }
        for (i, keyword) in self.keywords.iter().enumerate() {
            if i != 0 {
                result.push_str(", ");
            }
            result.push_str(&keyword.unparse(0));
        }
        result.push_str(")");
        result
    }
...
impl<R> Unparsable for ExprJoinedStr<R> {
    fn unparse(&self, _: usize) -> String {
        ...
        for value in &self.values {
            result.push_str(&value.unparse(0));
        }
        result.push_str("\"");
        result
    }
...
impl<R> Unparsable for ExprList<R> {
    fn unparse(&self, _: usize) -> String {
        let mut result = String::new();
        result.push_str("[");
        for (i, elt) in self.elts.iter().enumerate() {
...
impl<R> Unparsable for ExprTuple<R> {
    fn unparse(&self, _: usize) -> String {
        let mut result = String::new();
        result.push_str("(");
        for (i, elt) in self.elts.iter().enumerate() {
...
impl<R> Unparsable for ExprSlice<R> {
    fn unparse(&self, _: usize) -> String {
        ...
        if let Some(lower) = &self.lower {
            result.push_str(&lower.unparse(0));
        }
        result.push_str(":");
        if let Some(upper) = &self.upper {
            result.push_str(&upper.unparse(0));
        }
        if let Some(step) = &self.step {
            result.push_str(":");
            result.push_str(&step.unparse(0));
        }
        result.push_str("]");
        result
    }
...
impl<R> Unparsable for Comprehension<R> {
    fn unparse(&self, indent: usize) -> String {
        ...
        for if_ in &self.ifs {
            result.push_str(" if ");
            result.push_str(&if_.unparse(indent));
        }
        result
    }
}
...
impl<R> Unparsable for ExceptHandlerExceptHandler<R> {
    fn unparse(&self, indent: usize) -> String {
        ...
        if let Some(type_) = &self.type_ {
            result.push_str(" ");
            result.push_str(&type_.unparse(indent));
        }
...
impl<R> Unparsable for PythonArguments<R> {
    fn unparse(&self, indent: usize) -> String {
        ...
        for (arg, default) in self.kwonlyargs.iter().zip(&self.kw_defaults) {
            result.push_str(&arg.unparse(indent));
            result.push_str("=");
    ...
    fn unparse(&self, indent: usize) -> String {
        match self {
            ...
            Pattern::MatchSingleton(pattern_match_singleton) => {
                pattern_match_singleton.unparse(indent)
            }
            Pattern::MatchSequence(pattern_match_sequence) => {
                pattern_match_sequence.unparse(indent)
            }
            Pattern::MatchMapping(pattern_match_mapping) => pattern_match_mapping.unparse(indent),
            Pattern::MatchClass(pattern_match_class) => pattern_match_class.unparse(indent),
            Pattern::MatchStar(pattern_match_star) => pattern_match_star.unparse(indent),
            Pattern::MatchAs(pattern_match_as) => pattern_match_as.unparse(indent),
            Pattern::MatchOr(pattern_match_or) => pattern_match_or.unparse(indent),
        }
    }
}
...
impl<R> Unparsable for PatternMatchMapping<R> {
    fn unparse(&self, indent: usize) -> String {
        let mut result = String::new();
        result.push('{');
        for (key, pattern) in self.keys.iter().zip(&self.patterns) {
            result.push_str(&key.unparse(indent));
            result.push_str(": ");
            result.push_str(&pattern.unparse(indent));
            result.push_str(", ");
        }
...
impl<R> Unparsable for TypeParam<R> {
    fn unparse(&self, indent: usize) -> String {
        match self {
            ...
            TypeParam::ParamSpec(type_param_param_spec) => type_param_param_spec.unparse(indent),
            TypeParam::TypeVarTuple(type_param_type_var_tuple) => {
                type_param_type_var_tuple.unparse(indent)
            }
        }
    }
}
